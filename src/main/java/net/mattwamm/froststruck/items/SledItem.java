...
public class SledItem extends Item {
    ...
    SledEntity.Type type;
    public SledItem(SledEntity.Type type, Settings settings) {
        super(settings);
        this.type = type;
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        BlockHitResult hitResult = SledItem.raycast(world, user, RaycastContext.FluidHandling.ANY);
        ...
        if (((HitResult)hitResult).getType() == HitResult.Type.BLOCK) {
            SledEntity sledEntity = new SledEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z);
            sledEntity.setSledType(this.type);
            sledEntity.setYaw(user.getYaw());
            if (!world.isSpaceEmpty(sledEntity, sledEntity.getBoundingBox())) {
